...
#In-memory store for the clarification handler
# paused_clarifications: Dict[str, tuple[Clarification, Callable, str]] = {}
...
def on_step_start(plan: Plan, plan_run: PlanRun, step: Step) -> None:  # noqa: ARG001
  info = {
    ...
    "details": {
      ...
      "step_number": str(plan.steps.index(step)),
      "step_task": step.task
    }
  }
...
def on_plan_end(plan: Plan, plan_run: PlanRun) -> None:
  if plan_run.outputs.final_output:
    final = {
      "type": "ANSWER",
      "details": plan_run.outputs.final_output.value #type: ignore
    }
